...
const CollectionDetail = () => {
  ...
  return (
    <div className={`min-h-screen transition-all duration-700 ${isOffer ? 'bg-gradient-to-b from-rose-50 to-white' : isDeal ? 'bg-[#0a0c10]' : 'bg-gray-50'}`}>
      {/* Hero Section */}
      <div className={`pt-10 pb-20 px-4 ${isOffer ? 'bg-gradient-to-br from-rose-500 via-pink-600 to-fuchsia-700 text-white shadow-2xl shadow-pink-200 rounded-b-[4rem]' : isDeal ? 'bg-gradient-to-b from-gray-900 to-transparent text-white' : 'bg-white border-b border-gray-100'}`}>
        <div className="container mx-auto max-w-7xl">
          <button 
            onClick={() => navigate('/')}
            className={`flex items-center gap-2 mb-10 font-black text-[10px] uppercase tracking-widest px-5 py-2.5 rounded-full transition-all ${isOffer || isDeal ? 'bg-white/10 hover:bg-white/20 text-white backdrop-blur-md' : 'bg-gray-100 hover:bg-gray-200 text-gray-600'}`}
          >
        ...
        {collection.products.length > 0 ? (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 md:gap-8">
            {collection.products.map((product) => (
              <div 
                key={product._id} 
                className={`transition-all duration-500 hover:-translate-y-2 ${isOffer || isDeal ? 'bg-white/5 p-2 rounded-[2.5rem] border border-white/10 hover:border-white/30' : ''}`}
              >
                <ProductCard 
                  product={product} 
          ...
          <div className="text-center py-32 bg-white rounded-[3rem] shadow-xl border-2 border-gray-50">
            <p className="text-gray-400 font-bold text-xl">No products added to this collection yet.</p>
          </div>
        )}
      </div>
      ...
      <div className="container mx-auto max-w-7xl px-4 pb-20">
        <div className={`p-10 md:p-16 rounded-[4rem] text-center ${isOffer ? 'bg-rose-50 border-2 border-rose-100' : isDeal ? 'bg-gray-900 border border-white/5' : 'bg-gray-100'}`}>
          ...
          <button className={`px-10 py-4 rounded-2xl font-black text-sm uppercase tracking-widest transition-all ${isOffer ? 'bg-rose-600 text-white shadow-xl shadow-rose-200 hover:scale-105' : isDeal ? 'bg-white text-black hover:scale-105' : 'bg-gray-900 text-white hover:scale-105'}`}>
            Subscribe Now
          </button>
        </div>
      </div>
